...
func (m MemoInfo) tttt() {
    fmt.Println(m.MemoFormat)
}
...
func main() {
    ...
    mi := new(MemoInfo)//{"rr","tt","ttt"}
    //mi.MemoFormat = "lllllll"
    fmt.Println(mi)
    mi.tttt()

     bbbbb := getLedgerEntryType("AccountRoot")

     fmt.Println(reflect.TypeOf(bbbbb))
     fmt.Println(getTransactionType(lt))

     if v,ok := bbbbb.(int); ok {
        fmt.Println(reflect.TypeOf(v))
     }

    fmt.Println("======== >",uint32(32))

    fmt.Println("--------->",getLedgerEntryType(lt))
      
    fields := []string{"MemoData","MemoFormat","MemoType"}

    //m :=map[string]string{"type":"10","msg":"hello."}
    a := `{"msg":"hello.","type":10}`
mjson,_ :=json.Marshal(a)
...
fmt.Println(fields)
    ...
    for key, value := range fields {
        switch value {
            case "MemoType" :
            case "MemoFormat" :
                fmt.Println("MemoFormat --> ",value)
                break

        }

        fmt.Println(key,"==",value)//newMap[key] = value

    }

     //fmt.Println(GetFieldValue(mi,"MemoFormat"))
    //fmt.Println(fields)
    var zy1 uint8
    var zy uint8 = 1
    zy1 = zy << 4
    fmt.Println("======================= ",(0|56556), zy, zy1)
    bts,_ := HexToBytes("746573746466")
    fmt.Println(bts)
